                                    ...
                                    <tbody>
                                        ...
                                        if ($report_details) {
                                            ...
                                            foreach ($report_details as $dt) {
                                                ...
                                                echo '<td>' . $report_type[$dt->report_type] . '</td>';
                                                echo '<td>' . $dt->tot_dmd . '</td>';
                                                echo '<td>' . $dt->tot_colc . '</td>';
                                                echo '<td>' . $dt->recov_per . '</td>';
//                                                echo '<td>' . $dt->col_per . '</td>';
                                                if ($_SESSION['user_type'] == 'U') {
                                                    echo '<td><a href="report_entry/' . $dt->ardb_id . '?dt=' . $dt->return_dt . '" data-toggle="tooltip" data-placement="bottom" title="Edit"><i class="fa fa-pencil fa-lg"></i></a>';
                                                    if ($dt->fwd_data != 'Y') {
                                                        echo '<a href="delete/' . $dt->ardb_id . '?dt=' . $dt->return_dt . '" data-toggle="tooltip" data-placement="bottom" title="Delete" onclick="return check();"><i class="fa fa-trash-o fa-lg" style="color:red"></i></a>';
                                                    }
                                                    echo '</td>';
                                                    if ($dt->fwd_data != 'Y') {
                                                        echo '<td><a href="report_forward/' . $dt->ardb_id . '?dt=' . $dt->return_dt . '&sec=' . $dt->report_type . '" data-toggle="tooltip" data-placement="bottom" title="Forward"><i class="fa fa-arrow-circle-right fa-2x" style="color:green;"></i></a></td>';
                                                    } else {
                                                        echo '<td>' . $status . '</td>';
                                                    }
                                                } else {
