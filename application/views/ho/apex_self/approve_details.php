...
//                                                $pr_no = $dt->pronote_no;
    ...
    <script>
	function submit() {
	    window.location.href = "<?= site_url('/ho/apex_self/forward_data/' . $shg_details[0]->ardb_id . '/' . $memo_no); ?>";
	}
	function forward() {
	    var x = confirm("Are you sure you want to forward to APEX Bank?");
	    if (x)
		window.location.href = "<?= site_url('ho/apex_self/forward_to_apex/' . $shg_details[0]->ardb_id . '/' . $memo_no); ?>";
	    else
		return false;
	}
	function reject() {
	    window.location.href = "<?= site_url('/ho/apex_self/reject_data/' . $shg_details[0]->ardb_id . '/' . $memo_no); ?>";
	}
    </script>