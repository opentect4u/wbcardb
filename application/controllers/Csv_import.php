...
class Csv_import extends CI_Controller {
 ...
 function import_invest(){
    ...
    function dc()
	{	
		$data["export_details"]  = $this->Master->get_dc($this->session->userdata['login']->br_id);
		$this->load->view('common/header');
		$this->load->view('transaction/csv_dc_class',$data);
		$this->load->view('common/footer');

	}
    public function dc_upload(){

        $this->load->view('common/header');
		$this->load->view('transaction/dc');
		$this->load->view('common/footer');

	}
	function import_dc_csvfile(){
           ...
           //For Excel Upload
            ...
            if(!empty($_FILES['csv_file']['name']) && in_array($_FILES['csv_file']['type'],$csvMimes)){
                ...
                $csvFile = fopen($_FILES['csv_file']['tmp_name'], 'r');
				  ...
				  'disb_dt'	                 =>	$line[10],
				  'disb_amt'			     =>	$line[11],
				  'tot_project_cost'	     =>	$line[12],
				  'own_contr'                =>	$line[13],
				  'land_sec_area'		     =>	$line[14],
				  'cult_area'	             =>	$line[15],
				  'valuation_hypo'			 =>	$line[16],
				  'net_income'	             =>	$line[17],
				  'sec_amt'                  =>	$line[18],
				  'repayment_yr'			 =>	$line[19],
				  'memo_no'                  =>	$line[20],
				  'pronote_no'			     =>	$line[21],
				  'pronote_amt'	             =>	$line[22],
				  'intt_rt'                  =>	$line[23],
            ...
            $this->csv_import_model->f_insert_multiple('td_dc', $data);
            fclose($csvFile);
            //For notification storing message
            $this->session->set_flashdata('msg', 'Successfully added!');

            redirect('csv_import/dc');
    ...
    public function dc_delete(){
        ...
        $this->Master->delete_particulars("td_dc",$where);
