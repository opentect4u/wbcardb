...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Reports extends CI_Model{
		...
		public function f_get_fridayho($frmDt,$toDt){
            ...
            $query = $this->db->query("SELECT ardb_id,week_dt, ifnull(rd,0) rd, ifnull(fd,0) fd, ifnull(flexi_sb,0) flexi_sb, ifnull(mis,0) mis, ifnull(other_dep,0) other_dep, ifnull(ibsd,0) ibsd, ifnull(total_dep_mob,0) total_dep_mob, 
            	ifnull(cash_in_hand,0)  cash_in_hand, ifnull(other_bank,0) other_bank, ifnull(ibsd_loan,0) ibsd_loan, ifnull(dep_loan,0) dep_loan,ifnull(wbcardb_remit_slr,0) wbcardb_remit_slr,ifnull(wbcardb_remit_slr_excess,0) wbcardb_remit_slr_excess,      
                  ifnull(total_fund_deploy,0) total_fund_deploy
                                       FROM  td_fridy_rtn
									   where  week_dt between '$frmDt' and '$toDt'	   
									   order by ardb_id,week_dt;");
...
public function f_get_forthnight($frmDt,$toDt,$report_type){
	...
	public function get_dcardb($frmDt,$toDt,$ardb_id){

        $query = $this->db->query("SELECT ardb_id,brrower_name,sex,block_name,ifnull(round(sanc_amt/100000,2),0) sanc_amt,sanc_dt,recpt_dt,activity_name, loan_case_no,loan_bond_no,bond_dt,disb_dt,ifnull(round(disb_amt/100000,2),0)disb_amt,ifnull(round(tot_project_cost/100000,2),0)tot_project_cost,ifnull(round(own_contr/100000,2),0) own_contr,land_sec_area,
        	cult_area,ifnull(round(valuation_hypo/100000,2),0) valuation_hypo, ifnull(round(net_income/100000),0) net_income, ifnull(round(sec_amt/100000,2),0) sec_amt,repayment_yr,memo_no,pronote_no,pronote_amt,intt_rt  
                                   FROM  td_dc
								   where return_dt between '$frmDt' and '$toDt'	   
								   and   ardb_id  ='$ardb_id';");
