...
class Sale extends CI_Model{
 ...
 $data = $this->db->query("select cust_name,Sales_ID,trans_dt,sum(Net_Price) tot_amt
  ...
  public function js_get_batch_asPer_productSelection($product){
    ...
    $sql = $this->db->query("SELECT Batch FROM Inventory WHERE Prod_ID = '$product' Order by Expiry_dt");

    return $sql->result();

  }

  public function js_get_expiryasbatchSelection($Batch) {

    $sql = $this->db->query(" SELECT Expiry_dt, Max_Ret_Price, Units ,Qty  
      FROM Inventory WHERE Batch  = '$Batch' ");
  ...
  public function insert_sales($saleId,$user_id,$comp_id, $prodid, $Unit,$Batch,$Expiry,$Max_Ret_Price,$Dis_Rate, $dis_amount, $CGST_Rt,$qty,$Net_Price,$in_out_type,$br_cd, $cgst,$trans_dt,$cust_name,$doctor,$ph_no,$Unit_count)
  {
    ...
    for($j=0; $j<$Unit_count ; $j++)
    {
      ...
      $sql = "update Inventory set qty=qty - $qty[$j]  WHERE Batch='$Batch[$j]'";
      $this->db->query($sql);
